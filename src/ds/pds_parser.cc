...
void generator::context_insensitive(const string& initl,
	...
	const auto& top_initl = top_mapping(parser::parse_input_cfg(initl));

	const auto& cfsm_for_Z = parser::parse_input_cfsm(filename);
	const auto& approx_Z = context_insensitive(top_initl, cfsm_for_Z);

	const auto& cfsm_for_Y = parser::parse_input_cfsm(filename, true);
	const auto& approx_Y = context_insensitive(top_initl, cfsm_for_Y);
